...
class BillNumber extends TagLengthString
{
    ...
    {
        if (mb_strlen($value, 'UTF-8') > EMV::INVALID_LENGTH['BILL_NUMBER'] || $value === '') {
...
class StoreLabel extends TagLengthString
{
    public function __construct(string $tag, string $value)
    {
        if (mb_strlen($value, 'UTF-8') > EMV::INVALID_LENGTH['STORE_LABEL'] || $value === '') {
...
class TerminalLabel extends TagLengthString
{
    public function __construct(string $tag, string $value)
    {
        if (mb_strlen($value, 'UTF-8') > EMV::INVALID_LENGTH['TERMINAL_LABEL'] || $value === '') {
...
class MobileNumber extends TagLengthString
{
    public function __construct(string $tag, string $value)
    {
        if (mb_strlen($value, 'UTF-8') > EMV::INVALID_LENGTH['MOBILE_NUMBER'] || $value === '') {
...
class PurposeOfTransaction extends TagLengthString
{
    public function __construct(string $tag, string $value)
    {
        if (mb_strlen($value, 'UTF-8') > EMV::INVALID_LENGTH['PURPOSE_OF_TRANSACTION'] || $value === '') {
